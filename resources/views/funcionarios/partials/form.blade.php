...
<div class="col-md-4">
    ...
    <input type="text" class="form-control" id="cpf" name="cpf" minlength="11" maxlength="11"
...
<div class="col-md-4">
    ...
    <select id="departamento_id" name="departamento_id" class="form-select"
        value="{{ $funcionario->departamento->nome ?? '' }}" required>
        <option value="">--</option>
        @foreach ($departamentos as $departamento)
            <option value="{{ $departamento->id }}"
                @if (isset($funcionario->departamento_id)) @selected($funcionario->departamento->id == $departamento->id) @endif)>
    ...
    <select id="cargo_id" name="cargo_id" class="form-select" required>
        ...
        @foreach ($cargos as $cargo)
            <option value="{{ $cargo->id }}"
                @if (isset($funcionario->cargo->id)) @selected($funcionario->cargo->id == $cargo->id) @endif)>
